...
- In scope: a required `AgentName` Domain value, normalization and bounds validation, Agent provisioning and
  persistence of that name, and a safe-read contract update for [TASK-00021](00021-TASK.md).
...
- [x] The name is not present in raw-secret material, HMAC signing input, credential identity, or authorization
  checks.
- [x] The safe Agent read delivered by TASK-00021 exposes the name with lifecycle and credential metadata, while the
...
Agent or audit evidence. TASK-00021 owns the later safe-read projection of the retained name.
