...
Build the package through ordered user-valued slices. The first public `0.1.0` package milestone completes
invitation and activation, delivery recovery, login, session restoration and logout, user session management,
refresh continuity, password reset, and authenticated password change within the framework-neutral Domain and
Application boundary. `0.2.0` completes email change, disable and enable, deletion and restoration, and
pending-invitation correction. Later `0.x` milestones add the remaining administrative, authorization, realtime,
and hardening capabilities. `1.0.0` requires a separate stability decision.
...
- `0.1.0` requires invitation/activation, delivery recovery, login, cold-load restoration, logout, user session
  management, refresh continuity, password reset, and authenticated password change in the framework-neutral
  package and reusable conformance suite. Full implementation across the five starters follows tagged Fight
  AccessControl and Fight Common package releases and is not a `0.1.0` publication prerequisite.
...
- PRD-00016 owns repository authority and transfer. PRD-00018 owns framework-native starter delivery and human
  acceptance. This PRD owns only shared AccessControl behavior.
