...
Managed Roles and Permissions, required security audit evidence, stable public schemas, access-JWT orchestration,
and one editable React authentication client. It must do this without binding the shared Domain and Application
behavior to persistence, HTTP, framework security, queue, mail, or realtime implementations.
...
immutable token and read-model results, versioned public schemas, an editable React client, and a reusable
behavioral conformance suite. Consumer projects own persistence, HTTP, cookie, key, and runtime composition.
...
- The supported client profile keeps the short-lived access JWT only in React memory. The opaque refresh
  credential is carried by a secure HttpOnly cookie and stored only as a digest in authoritative server state.
  Remember-me changes refresh-session persistence and lifetime, not access-token lifetime or authority.
...
- Every full React load starts without an access JWT and awaits refresh before authenticated rendering. The
  client proactively refreshes at ten minutes, coordinates one shared refresh operation, and permits at most
  one eligible replayable request retry.
...
- Fight AccessControl creates access-JWT material through Fight Common security ports and supplies the shared
  editable React authentication lifecycle. The five starter repositories provide implementation evidence for
  real persistence, HTTP/cookies, key configuration, mail, queues, realtime, and client integration while all
  apply the same conformance outcomes.
